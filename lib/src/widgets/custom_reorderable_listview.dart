...
class CustomReorderableListView extends StatefulWidget {
  ...
  CustomReorderableListView({
    ...
    this.padding,
    this.reverse = false,
    this.useLongPressDraggable = true,
    this.dropAreaExtentScaleFactor = 1,
    this.dropAreaPlaceholderScaleFactor = 1,
  ...
  final double dropAreaExtentScaleFactor;
  final double dropAreaPlaceholderScaleFactor;
    ...
    _listOverlayEntry = OverlayEntry(
      ...
      builder: (BuildContext context) {
        return _ReorderableListContent(
          ...
          useLongPressDraggable: widget.useLongPressDraggable,
          dropAreaExtentScaleFactor: widget.dropAreaExtentScaleFactor,
          dropAreaPlaceholderScaleFactor: widget.dropAreaPlaceholderScaleFactor,
        );
      },
    );
  }
...
class _ReorderableListContent extends StatefulWidget {
  const _ReorderableListContent({
    ...
    @required this.useLongPressDraggable,
    @required this.dropAreaExtentScaleFactor,
    @required this.dropAreaPlaceholderScaleFactor,
  });
...
class _ReorderableListContentState extends State<_ReorderableListContent>
  ...
  static const double _defaultDropAreaExtent = 250.0;
  ...
  double get _dropAreaExtent {
    ...
    // CUSTOM CODE: making the drop area extent smaller so that
    // scrolling is less janky with large height draggables
    return (dropAreaWithoutMargin + _dropAreaMargin ) * widget.dropAreaExtentScaleFactor;
  }
  ...
  Widget _wrap(Widget toWrap, int index, BoxConstraints constraints) {
    ...
    Widget buildDragTarget(BuildContext context, List<Key> acceptedCandidates, List<dynamic> rejectedCandidates) {
      ...
      // CUSTOM CODE: add ability to switch between long press to drag
      // or just press to drag
      Widget child;
      if (widget.useLongPressDraggable) {
      ...
      switch (widget.scrollDirection) {
        ...
        case Axis.vertical:
        default:
          // CUSTOM CODE: make the sized box 1.5 times the dropareaextent,
          // which is actually 1/2 the size of widget being dragged
          // this results in a less janky scrolling behavior when
          // dragging items that have large heights (like more than 1/3 of the
          // screen)
          // also making the container grey to indicate where it will be dropped
          //
          spacing = SizedBox(height: _dropAreaExtent*widget.dropAreaPlaceholderScaleFactor,
            child: Container(
              color: Colors.grey,
            )
          );
